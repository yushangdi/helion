"""Compare RMSNorm forward implementations across backends on B200-class HW.

All impls compute the training-mode forward: y (input dtype), plus per-row
rstd in fp32 (what the backward pass consumes).

Implementations:

- ``aten``: eager ``torch.ops.aten._fused_rms_norm``
- ``compile``: ``torch.compile``'d fp32-upcast reference returning (y, rstd)
- ``quack``: Quack CuTe rmsnorm (analytical-heuristic config), direct
...
- ``helion-triton``: ``examples/rms_norm.py`` fwd kernel under
  ``HELION_BACKEND=triton``
- ``helion-cute``: same kernel under ``HELION_BACKEND=cute``

Methodology (same as ``benchmarks/cute/compare_layernorm_backends.py``):
...
- rmsnorm is memory bound: results are reported in GB/s
  (``2*M*N*elem + N*elem + M*4`` bytes per call: x read, y write,
  weight read, rstd write).

Examples::

    # Full comparison, one shape
    python benchmarks/cute/compare_rmsnorm_backends.py --m 32768 --n 8192

    # Single impl with cold full autotune, JSON to stdout
    python benchmarks/cute/compare_rmsnorm_backends.py --impl helion-cute \\
...
def _gbps(m: int, n: int, elem_bytes: int, ms: float) -> float:
    nbytes = 2 * m * n * elem_bytes + n * elem_bytes + m * 4
...
def _check(x, weight, y, rstd) -> None:  # noqa: ANN001
    import torch

    xf = x.float()
    ref_rstd = torch.rsqrt(xf.square().mean(dim=-1) + EPS)
    ref_y = xf * ref_rstd[:, None] * weight.float()
    torch.testing.assert_close(y.float(), ref_y, rtol=1e-2, atol=1e-2)
    # aten/quack return rstd shaped [M, 1] or [M]
    torch.testing.assert_close(rstd.float().reshape(-1), ref_rstd, rtol=1e-3, atol=1e-3)


def _quack_compiled_kernel(x, weight, config=None):  # noqa: ANN001, ANN202
    """Compile Quack's rmsnorm fwd (with rstd stored) and return a
    zero-overhead callable ``fn(x, w, out, rstd)``-closed over."""
    import torch
...
def _run_impl(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.impl == "aten":
        fn = lambda: torch.ops.aten._fused_rms_norm(x, [n], weight, EPS)  # noqa: E731
        y, rstd = fn()
    elif args.impl == "compile":

        def _ref(t, w):  # noqa: ANN001, ANN202
            tf = t.float()
            rstd = torch.rsqrt(tf.square().mean(dim=-1) + EPS)
            return (tf * rstd[:, None] * w.float()).to(t.dtype), rstd

        compiled = torch.compile(_ref)
        fn = lambda: compiled(x, weight)  # noqa: E731
        y, rstd = fn()
    elif args.impl == "quack":
        fn, y, rstd = _quack_compiled_kernel(x, weight)
        fn()
        ...
        for cfg in get_all_fwd_configs():
            if cfg.threads_per_row * cfg.cluster_n > n:
                continue
            try:
                cand_fn, *_ = _quack_compiled_kernel(x, weight, config=cfg)
                cand_fn()
                torch.cuda.synchronize()
            except Exception:
                continue
        ...
        fn, y, rstd = _quack_compiled_kernel(x, weight, config=best_cfg)
        fn()
